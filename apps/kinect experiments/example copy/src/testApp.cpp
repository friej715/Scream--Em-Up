...
void testApp::setup() {
    ...
    // set up the enemies--maybe a vector, maybe an array? depends on game type
    //    enemy.setup();
...
//        player1.xPos = (contourFinder.blobs[i].centroid.x) * gameScale;
//    }
...
bool testApp::compareYells() {
    ...
    for (int i = 0; i < oldYell.size(); i++) {
        ...
        //oldYellMax= MAX(oldYellMax, oldYell[i]);
    }
